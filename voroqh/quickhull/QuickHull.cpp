...
namespace quickhull {
	...
	HalfEdgeMesh<FloatType, size_t> QuickHull<FloatType>::getConvexHullAsMesh(const FloatType* vertexData, size_t vertexCount, bool CCW, FloatType epsilon) {
		...
		return HalfEdgeMesh<FloatType, size_t>(m_mesh, m_vertexData, m_faceMapping, m_halfEdgeMapping, m_vertexMapping);
	}
	...
	void QuickHull<T>::buildMesh(const VertexDataSource<T>& pointCloud, T epsilon)
	{
		...
		m_diagnostics.m_time_total_micro = std::chrono::duration<double, std::micro>(t_end - t_start).count();
	}
	...
	void QuickHull<T>::createConvexHalfEdgeMesh() {
		...
		m_diagnostics.m_time_setup_micro = std::chrono::duration<double, std::micro>(t_setup_end - t_setup_start).count();
		...
		m_diagnostics.m_time_process_micro = std::chrono::duration<double, std::micro>(t_process_end - t_setup_end).count();
